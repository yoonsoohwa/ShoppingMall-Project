...
export class CategoryList {
    ...
    renderCategoryList() {
        ...
        this.categoryItems.forEach(category => {
            let li = this.filterName === category.name ? xmlStringToDom(`<li class="category-item ${category.name}"><u>${category.name}</u></li>`) : xmlStringToDom(`<li class="category-item ${category.name}">${category.name}</li>`)
